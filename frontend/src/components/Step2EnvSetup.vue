              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  <input 
                    ...
                    :style="{ '--percent': customRoundsPercent + '%' }"
                  />
                  <div class="range-marks">
                    <span>{{ MIN_CUSTOM_ROUNDS }}</span>
                    <span 
                      class="mark-recommend" 
                      :class="{ active: customMaxRounds === RECOMMENDED_CUSTOM_ROUNDS }"
                      @click="customMaxRounds = RECOMMENDED_CUSTOM_ROUNDS"
                      :style="{ position: 'absolute', left: `calc(${recommendedRoundsPercent}% - 30px)` }"
                    >{{ $t('step2.recommendedRounds', { rounds: RECOMMENDED_CUSTOM_ROUNDS }) }}</span>
                    <span>{{ customRoundsMax }}</span>
                  </div>
                </div>
              </div>
...
const RECOMMENDED_CUSTOM_ROUNDS = 40
const useCustomRounds = ref(false) // 默认使用自动配置轮数
const customMaxRounds = ref(RECOMMENDED_CUSTOM_ROUNDS)   // 默认推荐40轮
...
const autoGeneratedRounds = computed(() => {
  ...
  return Math.max(calculatedRounds, RECOMMENDED_CUSTOM_ROUNDS)
})

const customRoundsMax = computed(() => autoGeneratedRounds.value || RECOMMENDED_CUSTOM_ROUNDS)

const normalizeCustomRounds = (rounds) => {
  const numericRounds = Number(rounds)
  const roundedRounds = Number.isFinite(numericRounds)
    ? Math.round(numericRounds)
    : MIN_CUSTOM_ROUNDS

  return Math.min(
    Math.max(roundedRounds, MIN_CUSTOM_ROUNDS),
    customRoundsMax.value
  )
}

const customRoundsPercent = computed(() => {
  if (customRoundsMax.value <= MIN_CUSTOM_ROUNDS) {
    return 100
  }

  return ((normalizeCustomRounds(customMaxRounds.value) - MIN_CUSTOM_ROUNDS)
    / (customRoundsMax.value - MIN_CUSTOM_ROUNDS)) * 100
})

const recommendedRoundsPercent = computed(() => {
  if (customRoundsMax.value <= MIN_CUSTOM_ROUNDS) {
    return 100
  }

  return ((normalizeCustomRounds(RECOMMENDED_CUSTOM_ROUNDS) - MIN_CUSTOM_ROUNDS)
    / (customRoundsMax.value - MIN_CUSTOM_ROUNDS)) * 100
})

watch([customMaxRounds, customRoundsMax], () => {
  const normalizedRounds = normalizeCustomRounds(customMaxRounds.value)
  if (customMaxRounds.value !== normalizedRounds) {
    customMaxRounds.value = normalizedRounds
  }
})
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    const rounds = normalizeCustomRounds(customMaxRounds.value)
    customMaxRounds.value = rounds
    params.maxRounds = rounds
    addLog(t('log.startSimCustomRounds', { rounds }))
  } else {
